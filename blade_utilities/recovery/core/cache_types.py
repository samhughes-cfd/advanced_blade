"""
Axis conventions for :class:`~blade_utilities.recovery.tensor_cache.cache.RecoveryCache`
(all stress-related arrays):

- **c**: load case index
- **s**: spanwise station index
- **p**: subcomponent row within the composite or isotropic routing table
- **k**: ply index (padded to ``n_ply_max`` for composites)
- **j**: beam resultant mode ``[N, My, Mz, T, Vy, Vz, B]``
- **q** (when used): Voigt ``[σ11, σ22, τ12]``
"""
...
class RecoveryCacheStorage:
    """Serializable fields for :class:`~blade_utilities.recovery.tensor_cache.cache.RecoveryCache`."""
