...
def _beam7_to_reference_forces6(res7: NDArray[np.float64]) -> NDArray[np.float64]:
    """Return the first six optimizer resultants in K6 RHS order.

    ``DesignEvaluator`` normalises all beam drivers to section/K7 order
    ``[N, My, Mz, T, Vy, Vz, B]`` before stress recovery.
    """

    r = np.asarray(res7, dtype=np.float64).ravel()
    if r.size < 6:
        return np.zeros(6, dtype=np.float64)
    return np.asarray(r[:6], dtype=np.float64)
