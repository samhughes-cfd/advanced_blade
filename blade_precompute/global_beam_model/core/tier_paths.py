...
**Tier A — Geometrically exact beam** (:mod:`global_beam_model.engine.solver`)
    Full nonlinear / linearised static solve on :class:`~global_beam_model.core.types.BeamModel`.
    Primary API: :class:`global_beam_model.api.BeamAnalysis`.
...
**Tier B — Prescribed section resultants** (:mod:`section_optimisation.engine.beam_k7`)
    ...
    from reference curvature via :func:`global_beam_model.engine.kinematics.rotmat_from_small_curvature`.
    Primary API: :class:`section_optimisation.engine.beam_k7.PrescribedResultantDriver`.
