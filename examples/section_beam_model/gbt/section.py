...
class WallDefinition:
    ...
    @property
    def length(self):
        return float(np.linalg.norm(self.node_end - self.node_start))

    @property
    def tangent(self):
        return (self.node_end - self.node_start) / self.length
...
class CrossSection:
    ...
    def _add_or_get_node(self, y, z, wall_id):
        for n in self._nodes:
            if abs(n.y - y) < self.node_tol and abs(n.z - z) < self.node_tol:
                if wall_id not in n.wall_ids:
                    n.wall_ids.append(wall_id)
                return n.node_id
        nid = len(self._nodes)
    ...
    def node_coordinates(self):
        return np.array([[n.y, n.z] for n in self._nodes])

    def get_strip(self, i):
    ...
    def strip_global_dofs(self, strip_id, n_dof_per_node):
        s = self._strips[strip_id]
        dm = self.dof_map(n_dof_per_node)
        return np.concatenate([dm[s.node_i], dm[s.node_j]])

    def strip_abd(self, i):
        return self._strips[i].material.abd_matrix()
    ...
    def strip_thickness(self, i):
        ...
        for a in ("t", "total_thickness"):
            if hasattr(m, a):
                return float(getattr(m, a))
        return 1e-3

    def all_abd_matrices(self):
        return [self.strip_abd(i) for i in range(self.n_strips)]
    ...
    def enclosed_area(self) -> float:
        for issue in self.validate():
            if "Dangling nodes" in issue:
                raise ValueError(
                    "enclosed_area() is only valid for closed sections. "
                    "This section has dangling (open) nodes."
                )
    ...
    def validate(self) -> list[str]:
        ...
        for i, w in enumerate(self.walls):
            if w.length < 1e-12:
                issues.append(f"Wall {i} zero length")
        ends = {}
        for s in self._strips:
        ...
        if dangle:
            issues.append(f"Dangling nodes: {dangle} (open section?)")
        return issues
    ...
    def summary(self) -> str:
        ...
        for i, w in enumerate(self.walls):
            lines.append(
                f"  Wall {i:2d} '{w.name}': length={w.length:.4f}m  n_strips={w.n_strips}  "
                f"mat={type(w.material).__name__}"
            )
        for iss in self.validate():
            lines.append(f"  ⚠ {iss}")
        return "\n".join(lines)
